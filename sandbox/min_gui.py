...
class Min_GUI(QtWidgets.QWidget):
    def __init__(self, parent: QtWidgets.QWidget = None, columns=col):
        ...
        self.form_elem = [None for i in range(len(columns))]
        for name in col:
            # Создадим в цикле QDoubleSpinBox по размерности
            # переданного списка для принятия от пользователя значений
            spin = QtWidgets.QDoubleSpinBox(self)
            spin.setObjectName('spin' + str(col.index(name)))
            spin.setDecimals(2)
            spin.setRange(0, 100000)
            spin.setSingleStep(0.01)
            spin.setSuffix("    " + name)
            layout.addRow(spin)
            self.form_elem[col.index(name)] = spin
        ...
        btn_calc = QtWidgets.QPushButton("Расчет")
        btn_calc.clicked.connect(self.btn_calc_func)
