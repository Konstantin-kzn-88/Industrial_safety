...
class RelationalTableModelWithIcon(QSqlRelationalTableModel):
    def __init__(self, tst_var="12", **kwargs):
        self.tst_var = tst_var
        QSqlRelationalTableModel.__init__(self, **kwargs)

    def data(self, index, role=Qt.DisplayRole):  # Переопределяем метод data
        if self.tst_var == "test":
            if index.column() == 1 and role == Qt.DisplayRole:  # для второго столбца, скроем выводимый текст
                return ""
            if index.column() == 1 and role == Qt.DecorationRole:  # и для него же выведем иконку в качестве декора
...
class Storage_app(QtWidgets.QMainWindow):
    ...
    def createModel(self):
        ...
        """
        self.model = RelationalTableModelWithIcon(db=self.db, tst_var = self.tst_var)
