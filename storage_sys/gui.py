...
from PySide2 import QtSql, QtGui, QtCore
...
class SQL_table_whith_ico(QtSql.QSqlRelationalTableModel):
    def __init__(self, select_table, **kwargs):
        super(SQL_table_whith_ico, self).__init__(**kwargs)
        self.select = select_table
        print(self.select)

    def data(self, index, role=QtCore.Qt.DisplayRole):  # Переопределяем метод data
        print(self.select)
        if self.select == "Компании":
            if index.column() == 11 and role == QtCore.Qt.DisplayRole:  # для второго столбца, скроем выводимый текст
                return ""
            if index.column() == 11 and role == QtCore.Qt.DecorationRole:  # и для него же выведем иконку в качестве декора
                return QtGui.QIcon(str(Path(os.getcwd()).parents[0]) + '/ico/project2.png')
...
class Storage_app(QtWidgets.QMainWindow):
    ...
    def createModel(self):
        ...
        """
        self.model = SQL_table_whith_ico(db=self.db, select_table="Экспертизы пром.безопасности")
