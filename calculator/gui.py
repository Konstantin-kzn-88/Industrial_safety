...
import random
from shapely.geometry import Point, LineString, Polygon  # TODO УБРАТЬ ЗАМЕНИТЬ НА СОБСТВЕННЫЙ МОДУЛЬ GEOMETRY?????
import xlwings as xw
import numpy as np
from PIL import Image  # TODO УДАЛИТЬ
...
class Painter(QtWidgets.QMainWindow):
    ...
    def draw_risk(self, data_list, sharpness=5):
        """Рисование риска"""
            ...
            for y in range(sharpness, height, sharpness):
                ...
                for obj in data_list:
                    ...
                    if len(coordinate) > 2:  # координаты можно преобразовать в полигон или линию
                        if type == 0:
                            # линейн. получим полигон
                            obj_coord = self.get_polyline_shapely(coordinate)
                            distance = int(round(Point(x, y).distance(obj_coord) * float(self.scale_plan.text())))
                        else:
                            # стац. об. получим полигон
                            obj_coord = self.get_polygon_shapely(coordinate)
                            distance = int(round(Point(x, y).distance(obj_coord) * float(self.scale_plan.text())))
                    else:  # не получается полигон, значит точка
                    ...
                    # 3.2 Пожар
                    if distance in strait_all_probit:
        ...
        blue = (0, 0, 255, 70)
        purple = (255, 0, 255, 70)
        img = Image.new(mode="RGB", size=(width, height), color=(255, 255, 255, 0))
        px = img.load()
